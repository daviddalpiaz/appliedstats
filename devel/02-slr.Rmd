...
- Calculate regression coefficients using `R`.
...
- Generate plots to visualize the relationship between two variables using `R`.
- Use `R` to simulate simple linear regression.

## Modeling

Let's consider a simple example of how the speed a car is traveling affects its stopping distance. That is, how far it travels before it comes to a stop. To examine this relationship, we will use the `cars` dataset which, is a default `R` dataset. Thus, we don't need to load a package first; it is immediately available.
...
The function $f$ describes the functional relationship between the two variables, and the $\epsilon$ term is used as an error term. This indicates that if we plug in a given value of $X$ as input, our output is a value of $Y$, within a certain range of error. You could think of this a number of ways:
...
where $\epsilon_i \sim N(0, \sigma^2)$. That is, the $\epsilon_i$ are *independent and identically distributed* (iid) normal random variables with mean $0$ and variance $\sigma^2$. This model has three parameters to be estimated: $\beta_0$, $\beta_1$, and $\sigma^2$.

Note that $X$ and $Y$ are no longer the same random variables as they appeared in the prior model representation. Now we are talking about applying that model to data that has been observed. The $x_i$ are considered fixed, known constants. The parameters $\beta_0$ and $\beta_1$ are also fixed, but unknown constants. The response $Y_i$ remains a random variable because of the random behavior of the error variable, $epsilon_i$. The subscript $i$ is now used to represent the paired nature of the observed data. That is, each response $Y_i$ is tied to an observable $x_i$ and a random, unobservable, $epsilon_i$.  

TODO: change or remove above or below now that above exists.

We have slightly modified our notation here, we are now using $Y_i$ and $x_i$, since we will be fitting this model to a set of $n$ data point, for $i = 1, 2, \ldots n$.
...
In the simple linear regression model, the $x_i$ are assumed to be fixed, known constants, and are thus notated with a lower case variable. The $Y_i$ are still considered random.

The random $Y_i$ are a function of $x_i$, thus we can write their mean and variance as a function of $x_i$,
...
Importantly, $E[Y_i]$ does depend on the value of $x_i$, while $Var[Y_i]$ does not.
...
- **N**ormal. The errors, $\epsilon$ are normally distributed. That is the "error" around the line follow a normal distribution.
...
- **Linear** tells us that are model for $Y$ is a linear combination of the predictors $X$. (In this case just the one.) Right now, this always results in a model that is a line, but later we will see how this is not always the case.
...
So SLR models $Y$ as a linear function of $X$, but how do we actually fine a good line? There are an infinite number of lines we could use, so we will attempt to find one with "small errors." That is a line with as many points as close to the line as possible. The questions now becomes, how do we find such a line? There are many approaches we could take.
...
The slope *parameter* $\beta_1$ tells us that for an increase in speed of one mile per hour, the **mean** stopping distance increases by $\beta_1$. It is important to specify that we are talking about the mean. Recall that $\beta_0 + \beta_1 x$ is the estimated mean of $Y$, in this case stopping distance, for a particular value of $x$. (In this case speed.) So $\beta_1$ tells us how the mean of $Y$ is affected by changing $x$.

Similarly, the *estimate* $\hat{\beta}_1 = `r round(beta_1_hat, 3)`$ tells us that for an increase in speed of one mile per hour, the **estimated** *mean* stopping distance increases by $`r round(beta_1_hat, 3)`$. Here we should be sure to specify we are discussing an estimated quantity. Recall that $\hat{y}$ is the estimated mean of $Y$ So $\hat{\beta}_1$ tells us how the estimated mean of $Y$ is affected by changing $x$. 

The intercept *parameter* $\beta_0$ tells us the **mean** stopping distance for a car traveling zero miles per hour. (Not moving.) The *estimate* $\hat{\beta}_0 = `r round(beta_0_hat, 3)`$ tells us that the **estimated** mean stopping distance for a car traveling zero miles per hour is $`r round(beta_0_hat, 3)`$. So when you apply the brakes to a car that is not moving, it moves backwards? This doesn't seem right. (Extrapolation, which we will see later, is the issue here.) 
...
We can now use this line to make predictions. First, let's see the possible $x$ values in the `cars` dataset. Since some $x$ values may appear more than once, we use the `unique()` function to only returns those values once.
...
Lastly, we can make a prediction for the stopping distance of a car traveling at 50 miles per hour. This is considered **extrapolation** as 21 is not an observed value of $x$ and is outside data range. We should be less confident in predictions of this type.
...
This is also an issue we saw when interpreting $\hat{\beta}_0 = `r round(beta_0_hat, 3)`$, which is equivalent to making a prediction at $x = 0$. We should not be confident in the linear relationship outside of the range of data we have observed.
...
This should be somewhat alarming or amazing. How is this true? For now we will leave this questions unanswered. (Think about this, and maybe try to prove it.) We will note define three of the quantities seen in this equation. 
...
For the `cars` example, we calculate $R^2 = `r round(R2, 3)`$. We then say that $`r round(R2, 3)*100`\%$ of the observed variability in stopping distance is explained by the linear relationship with speed.
...
The following three plots visually demonstrate the three "sums of squares" for a simulated dataset which has $R^2 = `r round(summary(lm(y ~ x, data = plot_data))$r.sq, 3)`$ which is a somewhat high value. Notice in the third plot, that the orange arrows account for a larger proportion of the total arrow.
...
The next three plots again visually demonstrate the three "sums of squares," this time for a simulated dataset which has $R^2 = `r round(summary(lm(y ~ x, data = plot_data))$r.sq, 3)`$. Notice in the third plot, that now the blue arrows account for a larger proportion of the total arrow.
...
The `lm()` command is used to fit **linear models** which actually accounts for a broader class of models than simple linear regression, but we will use SLR as our first demonstration of `lm()`. The `lm()` function will be one of our most recently used tools, so you may want to take a look at the documentation by using `?lm`. You'll notice there is a lot of information there, but we will start with just the very basics. This is documentation you will want to return to often.
...
The `abline()` function is used to add lines of the form $a + bx$ to a plot. (Hence **`ab`**`line`.) When we give it `stop_dist_model` as an argument, it automatically extracts the regression coefficients ($\hat{\beta}_0$ and $\hat{\beta}_1$) and uses them as the slope and intercept of the line. Here we also use `lwd` to modify the width of the line, as well as `col` to modify the color of the line.
...
An `R` function that is useful in many situations is `summary()`. We see that when it is called on our model, it returns a good deal of information. By the end of the course, you will know what every value here is used for. For now, you should immediately notice the coefficients, and you may recognize the $R^2$ value we saw earlier.
...
You may notice that this value was display above as a result of the `summary()` command, which `R` labeled the "Residual Standard Error."
...
The above code reads "predict the stopping distance of a car traveling 8 miles per hour using the `stop_dist_model`." Importantly, the second argument to the `predict()` is a data frame that we make in place. We do this so that we can specify that `8` is a value of `speed`, so that predict knows how to use it with the model stored in `stop_dist_model`. We see that this result is what we had calculated "by hand" previously.
...
Then, as is often the case with finding MLEs, for mathematical convenience we will take the natural logarithm of the likelihood function, then proceed to maximize the log-likelihood. The resulting estimates will be the same since log is a monotonically increasing function.
...
Next, we obtained simulated values of $\epsilon_i$.

```{r}
epsilon = rnorm(n, mean = 0, sd = sigma)
```

Now, since the $x_i$ values in SLR are considered fixed and known, we simply generate them from a uniform distribution. Know, that this is a arbitrary, but common practice. 
...
Now to check how well the method of least squares works, we use `lm()` to fit the model to our data, the take a look at the coefficients.
...
We should say here, that we're being sort of lazy here, and not the good kinda of lazy that could be considered efficient. Any time you simulate data, you should consider two things: writing a function, and storing the data in a data frame.
...
TODO: move unexplained source to explanation vs prediction?
...
TODO: car insurance explanation (shifting perspectives, also do this for braking)
