...
We use $i$ as an index, simply for notation. We use $x_i$ as the **predictor** (explanatory) variable. The predictor variable is used to help *predict* the **response** (target, outcome) variable, $y_i$. 

In the `cars` explain, we are interested in using the predictor variable `speed` to understand and predict the response variable `dist`.
...
TODO: move unexplained to explaination vs prediction?
...
TODO: use line to make predictions
...
TODO: extra formulas: SSReg = BHAT * SXX, RSS = SYY - SSReg
