<!-- In the future, this may be placed before regression, so the idea of discussing numeric predictors is not needed. The idea of a predictor may not have been introduced, so the focus should be on groups. -->
...
The design of experiments could be a course by itself. The Wikipedia article on [design of experiments](https://en.wikipedia.org/wiki/Design_of_experiments) gives a good overview. Originally, most of the methodology was developed for agricultural applications by [R. A. Fisher](https://en.wikipedia.org/wiki/Ronald_Fisher), but are still in use today, now in a wide variety of application areas. Notably, these methods have seen a resurgence as "A/B Testing."
...
where mean of each group is given by
...
\[
H_0: \mu_1 = \mu_2 = \ldots \mu_i \quad \text{vs} \quad H_1: \text{ Not all } \mu_i \text{ are equal.}
\]
...
|---------|--------------|----------------|------------|-------------|
| Between | SSB            | $g$                | SSB / DRB | MSB / MSW |
| Within  | SSW            | $n - g$            | SSW / DRW |             |
| Total   | SST            | $n - 1$            |             |             |

We reject the null (means equal) when the $F$ statistic is large. This occurs when the variation between groups is large compared to the variation within groups. Under the null hypothesis, the distribution of the test statistic is $F$ with degrees of freedom $g$ and $n - g$.
...
plot_anova = function(n = 20, mu_a = 0, mu_b = 0, mu_c = 0, sigma = 1) {
  ...
  #abline(h = mean(response[group == "A"]), lwd = 2, lty = 2, col = "dodgerblue")
  #abline(h = mean(response[group == "B"]), lwd = 2, lty = 2, col = "darkorange")
  #abline(h = mean(response[group == "C"]), lwd = 2, lty = 2, col = "black")
...
This should be a Shiny app.
Should output entire ANOVA table.
Do with ggplot2 and custom boxplots with show min, sd, mean, sd, max
```
...
Above, there isn't obvious separation between the groups like the first example, but it is still obvious the means are different. Below, there is more noise. Visually it is somewhat hard to tell, but the test suggests a difference of means.
...
Were we to run this experiment, we would have pre-specified a significance level. However, notice that the p-value of this test is incredible low, so using any reasonable significance level we would reject the null hypothesis. Thus we believe the diets had an effect on blood coagulation time.
...
- TODO: increase num_sims to 10000

Here we verify the distribution of the test statistic under the null hypothesis. We simulate from the null model (equal variance) to obtain an empirical distribution of the $F$ statistic, and compare it to the expected distribution.
...
f_stats = replicate(n = 100, sim_anova(stat = TRUE))
...
- Noise. The less noise, the test will more likely reject.
- Significance level $\alpha$. Lower significance level makes rejecting more difficult.
- Sample size. Large samples means easier to detect differences.
- Balanced design. An equal number of observations per group leads to higher power.

The following simulations look at the effect of $\alpha$, effect size, and noise level on the power of an ANOVA $F$-test. Homework will look into sample size and balance.

```{r}
p_vals = replicate(n = 10, sim_anova(mu_a = -1, mu_b = 0, mu_c = 0, mu_d = 1, 
...
```{r}
p_vals = replicate(n = 10, sim_anova(mu_a = -2, mu_b = 0, mu_c = 0, mu_d = 2, 
                                     sigma = 2.0, stat = FALSE))
mean(p_vals < 0.05)
mean(p_vals < 0.01)
```
...
Suppose we reject the null hypothesis from the ANOVA test for equal means. That tells us that the means are different, but which means? The obvious strategy is to test all possible comparisons of two means. We can do this easily in `R`.
...
- TODO; increase replication to 1000
...
# desired rate = 0.05
# no adjustment
mean(replicate(10, any(replicate(100, get_p_val()) < 0.05)))

# FWER with 100 tests
# desired rate = 0.05
# bonf adjustment
mean(replicate(10, any(p.adjust(replicate(100, get_p_val()), "bonferroni") < 0.05)))
```
...
We call this setup an $I \times J$ **factorial design** with $K$ replicates.

- $\alpha_i$ measures the effect of level $i$ of factor $A$. We call these the **main effects** of factor $A$.
- $\alpha_j$ measures the effect of level $j$ of factor $B$. We call these the **main effects** of factor $B$.
...
Under this setup, there are a number of models that we can compare. Consider a $2 \times 2$ factorial design. The following tables shows the means for each of the possible groups under each model.
...
- TODO: hierarchy
- TODO: the important thing is the difference between interaction and additive models. same number of means, but slight difference.

- TODO: don't fully write traditional ANOVA table. explain what is being compared. send to reference.

- TODO: which rows do which test
- TODO: which model is chosen?
...
- TODO: example with obvious interaction
...
```{r, echo = FALSE, eval = FALSE}
two-way calculation details in an appendix
```
