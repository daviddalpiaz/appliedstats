...
Above, there isn't obvious separation between the groups like the first example, but it is still obvious the means are different. Below, there is more noise. Visually it is somewhat hard to tell, but the test still suggests a difference of means. (At an $\alpha$ 05 0.05.)
...
Here, 48 rats were randomly assignmed both one of three poisons and one of four possible treatments. The experimenters then measures their survival time in tens of hours. A total of 12 groups, each with 4 replicates.
...
- TODO: parallel lines means no interaction. same effect
- TODO: what indicates interaction? is it significant?
...
These tests should be performed according to the model **hierarchy**.
...
- TODO: here we pick the interaction model
