...
`R` then reports the estiamte $\hat{y}$ (`fit`) for each, as well as the lower (`lwr`) and upper (`upr`) bounds for the interval, of a desired level.
...
However, we have to consider weight and year together now. And based on the above plot, one of the new cars is within the "blob" of observed values, while the other, the car from 1981 weighing 5000 pounds, is noticablely outside of the observed values. This is a hidden extrapolation which you should be aware of when using multiple regression.

We do a quick verification of soem of the mathematics in `R`.
...
With this in mind, confidence intervals for the individiual $\hat{\beta}_j$ are actually a special case of a confidence interval for mean response. 
...
The interpretation changes slightly, now in this case, we say that $`r round(100*summary(mpg_model)$r.squared, 2)`\%$ for the observed variation in miles per gallon is explained by the linear relationship with the two predictor varaibles, weight and year.
...
Notationally, we will denote the residuals of this model as $\hat{y}_{0i}$, which in this case happens to be:
...
We will denote the residuals of this model as $\hat{y}_{1i}$. The ANOVA table is then nearly idnetical to the ANOVA table from SLR, with two exceptions in the degrees of freedom column.
...
We see that the value of the $F$ statistic if `r anova(null_mpg_model, full_mpg_model)$F`, and the p-value is extremely low.

- TODO: null, neither wt nor year sig
- TODO: full, at least one
...
- TODO: null, none of the others sig 
- TODO: full, at least one
