...
As we saw in SLR, we can create confidence intervals for mean response, that is, an interval estimate for $E[Y]$. In SLR, the mean of $Y$ was only dependent on a single value $x$. Now, in multiple regression, $E[Y]$ is dependent on the value of each of the predictors, so we define the vector $x_0$ to be, 
...
Then our estimate of $E[Y]$ for a set of values $x_0$ is given by

\[
\hat{y} = x_{0}^\top\hat{\beta} = \hat{\beta}_0 + \hat{\beta}_1 x_{01} + \hat{\beta}_2 x_{02} + \cdots + \hat{\beta}_{p-1} x_{0(p-1)}.
\]

As with SLR, this is an unbiased estimate.

\[
E[\hat{y}] = x_{0}^\top\beta = \beta_0 + \beta_1 x_{01} + \beta_2 x_{02} + \cdots + \beta_{p-1} x_{0(p-1)}
\]
...
The math has changed a bit, but the process in `R` remains almost identical. Here we create a data frame for two additional cars. One can that weighs 3500 pounds produced in 1976, as well as a second car that weighs 5000 pounds which was produced in 1981.
...
We can then use the `predict()` function with `interval = "confidence"` to obtain intervals for both. Again, it is import to make the data passed to `newdata` a data frame, so that `R` knows which values are for which variables.

```{r}
predict(mpg_model, newdata = new_cars, interval = "confidence", level = 0.99)
```

`R` then reports the estimate $\hat{y}$ (`fit`) for each, as well as the lower (`lwr`) and upper (`upr`) bounds for the interval, of a desired level.
...
As are the year of each of the new cars.
...
We do a quick verification of some of the mathematics in `R`.
...
As with SLR, creating prediction intervals involves one slight change to the standard error to account for the factor that we are now considering an observation, instead of a mean.

Here we use $\hat{y}_{Pred}$ to estimate $Y_0$, the value of $Y$ for the predictor vector $x_0$

\[
\hat{y}_{Pred} = x_{0}^\top\hat{\beta} = \hat{\beta}_0 + \hat{\beta}_1 x_{01} + \hat{\beta}_2 x_{02} + \cdots + \hat{\beta}_{p-1}
\]

\[
E[\hat{y}_{Pred}] = x_{0}^\top\beta = \beta_0 + \beta_1 x_{01} + \beta_2 x_{02} + \cdots + \beta_{p-1} x_{0(p-1)} = Y_0
\]

\[
SE[\hat{y}_{Pred}] = s_e \sqrt{1 + x_{0}^\top\left(X^\top X\right)^{-1}x_{0}}
\]

\[
\hat{y}_{Pred} \pm t_{\alpha/2, n - p} \cdot s_e \sqrt{1 + x_{0}^\top\left(X^\top X\right)^{-1}x_{0}}
\]
...
The decomposition of variation that we had seen in SLR still remains true,

\[
\sum_{i=1}^{n}(y_i - \bar{y})^2 = \sum_{i=1}^{n}(y_i - \hat{y}_i)^2 + \sum_{i=1}^{n}(\hat{y}_i - \bar{y})^2.
\]

Which means that, we can still calculate $R^2$ in the same manner as before, which `R` continues to do automatically.

```{r}
summary(mpg_model)$r.squared
```

The interpretation changes slightly, now in this case, we say that $`r round(100*summary(mpg_model)$r.squared, 2)`\%$ for the observed variation in miles per gallon is explained by the linear relationship with the two predictor variables, weight and year.

We can also create the ANOVA table as before, and perform the significance of regression test. In multiple regression, the significance of regression test is
...
This is a model where the regression is insignificant. None of the predictors have a significant linear relationship with the response. Notationally, we will denote the residuals of this model as $\hat{y}_{0i}$, which in this case happens to be:
...
This is a model where the regression is significant. At least one of the predictors has a significant linear relationship with the response. We will denote the residuals of this model as $\hat{y}_{1i}$. The ANOVA table is then nearly identical to the ANOVA table from SLR, with two exceptions in the degrees of freedom column.
...
since we reject for large values of $F$. Here $F_{p-1, n-p}$ represents a random variable which follows an $F$ distribution with $p - 1$ and $n - p$ degrees of freedom.

To perform this test in `R`, we first explicitly specify the two models in `R` and save the results in different variables. We then use `anova()` to compare the two models, giving `anova()` the null first and the alternative (full) second.
...
First, notice that `R` does not display the results in the same manner as the table above. Importantly, we see that the value of the $F$ statistic is `r anova(null_mpg_model, full_mpg_model)$F[2]`, and the p-value is extremely low, so we reject the null hypothesis at any reasonable $\alpha$ and say that the regression is significant. At least one of `wt` or `year` has a useful linear relationship with `mpg`.
...
Also, note that, none of the individual $t$-tests are equivalent to the $F$-test as they were in SLR. This equivilence only holds for SLR because the individual test is the same as testing for all non-intercept parameters, since there is only one.

Here, we also verify the sums of squares and degrees of freedom directly in `R`. You should match these to the table from `R`, and use this to match `R`'s output to the written table above. 
...
The significance of the regression test is actually a special case of testing what we will call **nested models**. More generally we can compare two models, where one model is "nested" inside the other, meaning, one model contains a subset of the predictors from a lager model.

Conisder the following the full model,

\[
Y_i = \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + \cdots + \beta_p x_{i(p-1)} + \epsilon_i
\]

This model has $p - 1$ predictors, for a total of $p$ $\beta$-parameters. We will denote the residuals of this model as $\hat{y}_{1i}$.

Let, $q < p$, then define the null model,

\[
Y_i = \beta_0 + \beta_1 x_{i1} + \beta_2 x_{i2} + \cdots + \beta_{(q-1)} x_{i(q-1)} + \epsilon_i
\]

This model has $q - 1$ predictors, for a total of $q$ $\beta$-parameters. We will denote the residuals of this model as $\hat{y}_{0i}$.

The difference between these two models can be codified by the null hpothesis of a test.
...
\[
F = \frac{\sum_{i=1}^{n}(\hat{y}_{1i} - \hat{y}_{0i})^2 / (p - 1)}{\sum_{i=1}^{n}(y_i - \hat{y}_{1i})^2 / (n - p)}.
\]
...
For example, the `autompg` dataset has a number of additonal varaibles that we have yet to use.

```{r}
names(autompg)
```

We'll contain to use `mpg` as the response, but now we will consider two different models.
...
Here we have used the formula `mpg ~ .` to define to full model. This is the same as the commented out line. Specifically, this a common shortcut in `R` which read "model `mpg` as the response with each of the remaing variables in the data frame as predictors."

Here we see that the value of the $F$ statistic is `r anova(null_mpg_model, full_mpg_model)$F[2]`, and the p-value is very large, so we fail to reject the null hypothesis at any reasonable $\alpha$ and say that none of `cyl`, `disp`, `hp`, and `acc` are significant with `wt` and `year` already in the model.
...
Looks good! Normal: Check. Center at 6: Check. Spread of about 0.15: Check.
