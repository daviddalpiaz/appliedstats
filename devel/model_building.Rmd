...
So here, dispite having multiple predictors, we chose to use only one. How is this model **fit**? We will almost exclusively use the method of least squares, but recall, we had seen alternative methods of fitting this model.
...
Any of these methods (we will always uses the last, least squares) will obtain estimates of the unknown parameters $\beta_0$ adn $\beta_1$. Since those are the only unkowns of the specified model, we have then *fit* the model. The fitted model is then
...
Often, we'll use multiple predictors in out model. Very often, we will at least try a model with all possible predictors.
...
TODO: 
- family: `lm()`, `glm()`, non-parametric (smooth, trees, etc), etc (certain class of restrictions, in our case, linear combinatinos of predictors)
...
TOODO: To find small and interpretable models, we would use selection criterion that *explicitly* penalize larger models, such as AIC and BIC. In this case we still obtained a somewhat large model, but much smaller than the model we used to start the selection process. __Eventually we will do this. For now, we need to use ANOVA() in an ad-hoc manner. For inference, we need an extra assumption. Normallity of errors. We will discuss this is great detail later.__
