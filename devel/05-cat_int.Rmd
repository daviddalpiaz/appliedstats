...
So far we have limited our use of categorical variables to binary categorical variables. Specifically, we have limited ourselves to dummy variables, which take values `0` or `1`, which represent a categorical variable numerically.

We'll now discuss **factor** variables, which is a special way that `R` deals with categorical variables so that a human user can simply think about the categories of a categorical variable, and `R` will take care of the necessary dummy variables.

```{r}
is.factor(autompg$domestic)
```

Before when we used the `domestic` variable that was created, it was **not** a factor variable. It was simply a numerical variable that only took two possible values, `1` for domestic, and `0` for foreign. Let's create a new variable `origin` that stores the same information, but in a different way.
...
Now the `origin` variables stores `"domestic"` for domestic cars, and `"foreign"` for foreign cars.

```{r}
is.factor(autompg$origin)
```

However, this is simply a vector of characters. We'll want to *coerce* this variable to be a factor.
...
It seems that it doesn't produce the same results. Right away we notice that the intercept is different, as is the the coefficient in front of `disp`. We also notice that the remaining two coefficients are the same magnitude, but with a different sign. Why is this happening?
...
Here the `cyl` variable has three possible levels: `4`, `6`, and `8`. You may wonder, why not simply use `cyl` as a numerical variable? You certainly could. However, that would force the difference between `4` and `6` cylinders to be the same as the difference between `6` and `8` cylinders. For this reason, we will simply consider them to be categories. This is a decision that will often need to be made with ordinal variables. Often, with a large number of categories, the decision to instead use them as numerical variables is appropriate, because, as we are about to see, a number of dummy variables are create to represent these variables.
...
Now, let's fit an additive model in `R`, using `mpg` as the response, and `disp` and `cyl` as predictors. This should be a model that uses "three lines" to model `mpg`, one for each of the possible `cyl` levels. They will all have the same slope (since it is an additive model) but each will have its own intercept.
...
The odd result here is that we're estimating that 8 cylinder cars have better fuel efficiency than 6 cylinder cars at **any** displacement! That doesn't seem right. Maybe for very large displacement engines that could be true, but that seems wrong for medium to low displacement.
...
To completely justify the interaction model (slope for each `cyl` level), compared to the additive model (single slope), we can perform an $F$-test. Notice first, that there is no $t$-test that will be able to do this since the difference between the two models is not a single parameter.
...
As expected, we see a very low p-value, thus reject the null, and we prefer the interaction model over the additive model.
...
We see there is a difference of two parameters, which is also displayed in the resulting ANOVA table from `R`. Also notice that the following two values also appear on the ANOVA table.
...
A quick note here: the full model does still "fit better.". Notice that it has a smaller RMSE, it makes smaller squared errors on average.
...
However, it is not much smaller. We could even say that, the difference is insignificant. This is an idea we will return to later in greater deatil.
