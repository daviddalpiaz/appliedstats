...
- Create interval estimates for regression parameters.
...
Recall that tests for means often take the form:
...
The `names()` function tells us what information is available, and then we use the `$` operator and `ccoefficients` to extract the information we are interested in. Two values here should be immediately familiar.
...
\[
SE[\hat{\beta}_1] = \frac{s_e}{\sqrt{S_{xx}}} = `r summary(stop_dist_model)$coefficients[2,2]`.
\]

The third value, `t value`, is the value of the test statistics for testing $H_0: \beta_1 = 0$ vs $H_1: \beta_1 \neq 0$,

\[
t = \frac{\hat{\beta}_1-0}{SE[\hat{\beta}_1]} = \frac{\hat{\beta}_1-0}{s_e / \sqrt{S_{xx}}} = `r summary(stop_dist_model)$coefficients[2,3]`.
\]
...
In summary, the following code stores the information of `summary(stop_dist_model)$coefficients` in a new variable `stop_dist_model_test_info`, then extracts each element into a new variables which describes the information.
...
In this plot of simulated data, we see a clear relationship between $x$ and $y$, however it is not a linear relationship. If we fit a line to this data, it is very flat. The resulting test for $H_0: \beta_1 = 0$ \text{vs} $H_1: \beta_1 \neq 0$ gives a large p-value, in this case $`r summary(lm(y ~ x))$coef[2,4]`$, so we would fail to reject and say that there is no significant linear relationship between $x$ and $y$. We will see later how to fit a "linear" model to this data, but for now, realize that testing $H_0: \beta_1 = 0$ vs $H_1: \beta_1 \neq 0$ can only detect straight line relationships.
...
Note that this confidence interval does **not** contain the hypothesized value of 0, which is equivalent to rejecting the test of $H_0: \beta_1 = 0$ vs $H_1: \beta_1 \neq 0$.

You should be somewhat suspicious of the confidence interval for $\beta_0$, as it covers negative values, which correspond to negative stopping distances. Technically the interpretation would be that we are 99% confident that the average stopping distance of a car traveling 0 miles per hour is between `r confint(stop_dist_model, level = 0.99)[1,1]` and `r confint(stop_dist_model, level = 0.99)[1,2]`, but we don't really believe that, since we are actually certain that it would be non-negative.

Note, we can extract specific values from this output a number of ways. This code is not run, instead you should check how it relates to the output of the code above.
...
In addition to confidence intervals for $\beta_0$ and $\beta_1$ there two other common interval estimates used with regression. The first is called a **confidence interval for the mean response**. Often, we would like an interval estimate for the mean, $E[Y]$ for a particular value of $x$.

In this situation we use $\hat{y}$ as our estimate of $E[Y]$.
...
Thus, $\hat{y}$ is a good estimate since,
...
Like the other estimates we have seen, $\hat{y}$ also follows a normal distribution,

\[
\hat{y} \sim N \left(\beta_0 + \beta_1 x, \sigma^2 \left(\frac{1}{n}+\frac{(x-\bar{x})^2}{S_{xx}}\right) \right).
\]

And the lastly, since we need to estimate this variance, we arrive at the standard error of our estimate,
...
To find confidence intervals for the mean response using `R`, we use the `predict()` function. We give the function our fitted model as well as new data, stored as a data frame. (This is important, so that `R` knows the name of the predictor variable.)
...
Sometimes we would like an interval estimate for a new observation, $Y$ for a particular value of $x$. This is very similar to an interval for the mean response, $E[Y]$, but different in one very important way.

Our best guess for a new observation is still $\hat{y}$. The estimated mean is still the best prediction we can make. The difference is in the amount of variability, since we know that observations will vary about the true regression line according to a $N(0, \sigma^2)$ distribution. Because of this we add an extra factor of $\sigma^2$ to our estimates variability.

\[
Var[\hat{y}] = \sigma^2 \left(1 + \frac{1}{n}+\frac{(x-\bar{x})^2}{S_{xx}}\right)
\]
...
\[
SE[\hat{y}] = s_e \sqrt{1 + \frac{1}{n}+\frac{(x-\bar{x})^2}{S_{xx}}}
\]
...
Also notice that these two intervals are wider than the corresponding intervals for the mean response.
...
- We use the `ylim` argument to stretch the plot, since the bands extend further than the points.
...
- The prediction bands are less curved than the confidence bands. This is a result of the extra factor of $\sigma^2$ added to the variance at any value of $x$.
...
which follows an $F$ distribution with degrees of freedom $1$ and $n - 2$ under the null hypothesis. An $F$ distribution is a continuous distribution which take only positive values and has two parameters, which are the two degrees of freedom.
...
When can then think of this usage of `anova()` as directly comparing the two models. (Notice we get the same p-value again.)