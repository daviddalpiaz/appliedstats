...
Moreover, by calling `sum()` on a vector of logical values, it automatically converts them to `0` and `1` and performs the same operation as above. This is an extremely useful feature.
...
Note however that, without using the argument names, the order matters, as the following code will not evaluate to the same output.
...
We see the function is working as expected, and when returning the unbiased estimate it matches `R`'s built in function `sd()`.
...
Also, recall that for a random variable $X$ with finite mean $\mu$ and finite variance $\sigma^2$, the central limit theorem tells us that the mean, $\bar{X}$ of a random sample of size $n$ is approximately normal for *large* values of $n$. Specifically,
