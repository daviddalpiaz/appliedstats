...
Early we looked at installing packages, in particular the `ggplot2` package. (A package for visualization. While not necessary for this course, it is quickly growing in popularity.) Inside the `ggplot2` package is a dataset called `mpg`.
...
- Simulate a sample of 25 observations from $N(\mu_1 = 6,\sigma^2 = 4)$. Call the mean of this sample $\bar{x}_{1s}$.  
- Simulate a sample of 25 observations from $N(\mu_1 = 5,\sigma^2 = 4)$. Call the mean of this sample $\bar{x}_{2s}$.  
- Calculate the differences of the means, $d_s = \bar{x}_{1s} - \bar{x}_{2s}$.

We will repeat the process a large number of times. Then we ill use the distribution of the simulated observations $d_s$ as an estimate for the true distribution of $D$.
...
We could have also accomplished most of this task with a single line of more "idomatic" `R`.
...
Use `?replicate` to take a look at the documentation for the `replicate` and is if you can understand how this line performs the same operations that our `for` loop above executed.

```{r}
mean(differences == diffs)
```

Here we see that by setting the same seed for the randomization, we actually obtain identical results!
