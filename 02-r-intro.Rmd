...
There are many good resoruces for learning `R`. They are not necessary for this course, but you may find them useful if you would like a deeper understanding of `R`:
...
- [The Art of `R` Programming](https://www.nostarch.com/artofr.htm) by Norman Matloff
    - Gentle introduction to the programming side of `R`. (Whereas we will focus more on the data analysis side.) Free electronic version available through Illinois library.
...
`R`'s most most basic function is that of a simple calculator.
...
TODO: Note about [1] in output. Output a big vector?
