...
- **Regression** simply means that we are attempting to measure the relationship between a *numeric* response variable and (one or more) predictor variables.
...
\frac{\text{SSReg}}{\text{SST}} = 
1 - \frac{\sum_{i=1}^{n}(y_i - \hat{y}_i)^2}{\sum_{i=1}^{n}(y_i - \bar{y})^2} = 
1 - \frac{\sum_{i = 1}^{n}e_i^2}{\sum_{i=1}^{n}(y_i - \bar{y})^2} = 1 - \frac{\text{SSE}}{\text{SST}}
$$
...
For the `cars` example, we calculate $R^2 = `r round(R2, 2)`$. We then say that $`r round(R2, 3)*100`\%$ of the observed variability in stopping distance is explained by the linear relationship with speed.

```{r, echo = FALSE, message = FALSE, warning = FALSE}
## TODO: this code can be made much better
...
The following three plots visually demonstrate the three "sums of squares" for a simulated dataset which has $R^2 = `r round(summary(lm(y ~ x, data = plot_data))$r.sq, 2)`$ which is a somewhat high value. Notice in the third plot, that the orange arrows account for a larger proportion of the total arrow.
...
The next three plots again visually demonstrate the three "sums of squares," this time for a simulated dataset which has $R^2 = `r round(summary(lm(y ~ x, data = plot_data))$r.sq, 2)`$. Notice in the third plot, that now the blue arrows account for a larger proportion of the total arrow.
