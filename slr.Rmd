...
The function $f$ describes the functional relationship between the two variables, and the $\epsilon$ term (epsilon) is used to account for error. This indicates that if we plug in a given value of $X$ as input, our output is a value of $Y$, within a certain range of error. You could think of this a number of ways:
...
We could try to model the data with a horizontal line. That is, the model for $y$ does not depend on the value of $x$ and is a constant. (Some function $f(X) = c$.) The plot below shows that this doesn't do a very good job. Many of the data points are far from away the orange line representing $c$. This is an example of **underfitting**. The obvious fix is to make the function $f(X)$ actually depend on $x$.
...
Lastly, we could try to model the data with a well chosen line rather than one of the two extremes previously attempted. The line on the plot below summarizes the relationship between stopping distance and speed quite well. As speed increases, the distance required to come to a stop increases. There is still some variation about this line, but it seems to capture the overall trend.
...
Then, as is often the case when finding MLEs, for mathematical convenience we will take the natural logarithm of the likelihood function. Taking the log lets us break up a product of terms into a sum of terms. A sum of terms is easy to differentiate and therefore find the maximum. Since log is a monotonically increasing function, maximizing the log-likelihood results in the same estimate as if we had not taken the log.
...
The RMarkdown file for this chapter can be found [**here**](slr.Rmd). The file was created using `R` version `r paste0(version$major, "." ,version$minor)` and the following packages:
