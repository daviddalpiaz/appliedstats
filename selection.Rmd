...
\[
\text{AIC} \approx n\log\left(\frac{\text{RSS}}{n}\right) + 2p
\]
...
\[
\text{BIC} \approx n\log\left(\frac{\text{RSS}}{n}\right) + \log(n)p
\]
...
Let's calculate LOOCV $\text{RMSE}$ for both models, then discuss *why* we want to do so. We first write a function which calculates the LOOCV RMSE as defined using the shortcut formula for linear models.
...
\[
AIC \approx n\log\left(\frac{\text{RSS}}{n}\right) + 2p,
\]
...
We see that every row above `<none>` has a smaller $\text{AIC}$ than the row for `<none>` with the one at the top, `Ht`, giving the lowest AIC. Thus we remove `Ht` from the model, and continue the process.
...
We'll use the approximation of $\text{AIC}$ which leaves of the constant term that is equal across all models.
...
The `extractAIC()` function will calculate the $\text{AIC}$ approximation defined above for a fitted model.
...
We'll further discuss the model `autompg_mod_back_bic` to better explain the difference between using models for *explaining* and *predicting*. This is the model fit to the `autompg` data that was chosen using Backwards Search and BIC, which obtained the lowest LOOCV $\text{RMSE}$ of the models we considered.
